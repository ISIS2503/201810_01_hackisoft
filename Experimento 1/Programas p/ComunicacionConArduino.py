...
@app.route('/nuevocodigo/<string:id>/<string:codigo>/<string:hi>/<string:hf>', methods=['POST'])
def post_nuevocodigo(id, codigo, hi, hf):
    m = list(codigo)
    if(len(m) != 4):
        return make_response("El código debe ser de 4 dígitos!", 400)
    if request.method == 'POST':
        mqtt.publish("UnidadResidencial/Inmueble/Configuracion", "crear:"+id+":"+codigo+":"+hi+":"+hf+"\n")
        return "Código añadido!"

@app.route('/actualizarcodigo/<string:id>/<string:codigo1>/<string:hi>/<string:hf>' , methods=['PUT'])
def put_actualizacodigo(id, codigo1, hi, hf):
    m = list(codigo1)
    if (len(m) != 4 ):
        return make_response("El código debe ser de 4 dígitos!", 400)
    if request.method == 'PUT':
        mqtt.publish("UnidadResidencial/Inmueble/Configuracion", "actualizar:"+id+":"+codigo1+":"+hi+":"+hf+"\n")
        return "Código actualizado!"

@app.route('/eliminarcodigo/<string:id>', methods=['DELETE'])
def delete_eliminarcodigo(id):
    if request.method == 'DELETE':
        mqtt.publish("UnidadResidencial/Inmueble/Configuracion", "eliminar:"+id)
